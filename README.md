...
`!afk` (or `css_afk` in console) pauses idle checks on yourself for the configured time. The warning message mentions it when the command is enabled. Typing it does not show up in chat.
